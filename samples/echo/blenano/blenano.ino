...
void writtenHandle(const GattWriteCallbackParams *Handler)
{
  if(Handler->handle == tx_characteristic.getValueAttribute().getHandle()){
    uint16_t buf_len;
    ble.readCharacteristicValue(tx_characteristic.getValueAttribute().getHandle(), buf, &buf_len);
    ...
    // echo
    ble.updateCharacteristicValue(rx_characteristic.getValueAttribute().getHandle(), (const uint8_t *)"echo>", 5);
    ble.updateCharacteristicValue(rx_characteristic.getValueAttribute().getHandle(), buf, buf_len);
  }
}
